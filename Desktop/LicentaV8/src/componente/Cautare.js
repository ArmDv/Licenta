...
import { filterData,offersData } from '../global/Data';
...
const handleSearch = text =>{
    const dataS = filter(offersData, userSearch =>{        //       (2)
            ...
            renderItem={({ item }) => (
              <TouchableOpacity 
                       onPress = {() =>{
                            ...
                            Keyboard.dismiss
                            navigation.navigate("ResultScreen",{item:item.name})
                            setModalVisible(false)
                            setTextInputFossued(true)
                                }} >
